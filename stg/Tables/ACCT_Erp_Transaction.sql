...
GO
ALTER INDEX [UQ_ACCT_Erp_Transaction_Merge]
    ON [stg].[ACCT_Erp_Transaction] DISABLE;


GO

CREATE NONCLUSTERED INDEX [IX_ACCT_Erp_Transaction_Organization_Lookup]
    ON [stg].[ACCT_Erp_Transaction]([SourceSystemCode] ASC, [SourceSystemGlobalDimension1Ref] ASC, [CompanyCode] ASC);


GO
ALTER INDEX [IX_ACCT_Erp_Transaction_Organization_Lookup]
    ON [stg].[ACCT_Erp_Transaction] DISABLE;


GO

CREATE NONCLUSTERED INDEX [IX_ACCT_Erp_Transaction_PostingType_Lookup]
    ON [stg].[ACCT_Erp_Transaction]([PostingType] ASC)
    INCLUDE([Date], [Voucher], [TransactionNo]);


GO
ALTER INDEX [IX_ACCT_Erp_Transaction_PostingType_Lookup]
    ON [stg].[ACCT_Erp_Transaction] DISABLE;
