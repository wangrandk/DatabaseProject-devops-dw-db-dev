CREATE TABLE [fact].[QA_ServiceQuality] (
    [Id]                        BIGINT          NOT NULL,
    ...
    CONSTRAINT [PK_QA_ServiceQuality] PRIMARY KEY CLUSTERED ([RespondentId] ASC, [SurveyQuestionKey] ASC)
);


GO

CREATE UNIQUE NONCLUSTERED INDEX [UQ_QA_ServiceQuality_Update]
    ON [fact].[QA_ServiceQuality]([RespondentId] ASC, [DateKey] ASC, [SurveyQuestionKey] ASC);


GO

CREATE COLUMNSTORE INDEX [CSIX_QA_ServiceQuality]
    ON [fact].[QA_ServiceQuality]([InterviewerId], [DateKey], [MeasuringPeriodKey], [Purpose], [LineKey], [SurveyQuestionKey], [VehicleRef], [Age], [Gender], [DurationInMinutes], [MinTarget], [Importance], [Placement], [Language], [InterviewerStartStopKey], [InterviewerEndStopKey], [HighBoundary], [IsZeroed], [SurveyAnswerKey], [ContractKey], [ContractorKey], [GarageKey], [Frequency], [MeasuringPeriod], [NytBynetPoints], [NytBynet], [MaxSetOff], [LowBoundary]);
